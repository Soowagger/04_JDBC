...
public class MemberView {
		...
		try {
			...
			for(Member member : memberList) {
				System.out.printf("%d\t\t%s\t\t%s\t\t%s\n", 
								index+1, member.getMemberId(), member.getMemberName(), member.getMemberGender());
				index++;
			}
	...
	public void updateMember() {
		...
		try {
			...
			// -> 그래야 누구를 update 할 지 조건문에 쓸 수가 있음
			
			if(result > 0) {
				System.out.println("\n=== 수정 되었습니다. ===\n");
				
				// Service를 호출해서 현재 DB만 수정해둔 상태
	...
	public void updatePassword() {
		...
		while(true) {
			...
			if(newMemberPw.equals(pwConfirm)) {
				break;
			}
		...
		try {
			int result = service.updatePassword(newMemberPw, Session.loginMember.getMemberPw(), 
												Session.loginMember.getMemberNo());
			
			if(result > 0) {
				System.out.println("\n==== 비밀번호가 변경되었습니다.====");
				Session.loginMember.setMemberPw(newMemberPw);
			} else {
				System.out.println("\n**** 비밀번호 변경 실패 ****\n");
			}
